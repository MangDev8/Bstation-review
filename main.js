...
async function initFCM() {
    ...
    try {
        ...
        // Kirim token ke server jika mau push notifikasi realtime
    } catch (err) {
...
async function loadNotif() {
    try{
        ...
        data = data.filter(item=>item.enabled && (!item.expire || (now - parseInt(localStorage.getItem("notif_"+btoa(item.text))||now))/3600000<=item.expire));
...
document.addEventListener("DOMContentLoaded", ()=>{
    loadNotif();
    setInterval(loadNotif,5000); // cek notif setiap 5 detik
});

// ================= Modal notif =================
window.openNotif = function(){ document.getElementById("notifModal").style.display="flex"; badge.style.display="none"; badge.textContent=""; };
